...
if (isset($_POST['godown-Submit'])) {
	...
	if (empty($Hlocation) || empty($Hprice) || empty($Hcontact) || empty($Harea) || empty($Hdiscription)) {
     ...
     elseif (!preg_match("/^[+0-9]*$/", $Hcontact)) {
        header("Location: ../Uploadland_Admin.php?error=invalidcontact");
        exit();
}
    ...
    elseif (!preg_match("/^[0-9]*$/", $Hprice)) {
        header("Location: ../Uploadland_Admin.php?error=invalidprice");
        exit();
}
    elseif (!preg_match("/^[0-9]*$/", $Harea)) {
        header("Location: ../Uploadland_Admin.php?error=invalidinputs");
        exit();
}
else {
	...
	if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../Uploadland_Admin.php?error=sqlerror");
		exit();
	}
	else {
		session_start();
		...
		header("Location: ../Uploadland_Admin.php?upload=success");
		exit();
	}
