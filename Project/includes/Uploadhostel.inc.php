...
if (isset($_POST['hostel-Submit'])) {
	...
	if (empty($Hstatus) || empty($Helectric) || empty($Hwater) || empty($Hfence) || empty($Hself) || empty($Hfood) || empty($Hcity) || empty($Hdistrict) || empty($Hlocality) || empty($Hprice) || empty($Hcontact) || empty($Hdiscription)) {
		header("Location: ../Uploadhostel.php?error=emptyfield");
		exit();
	}
    elseif (!preg_match("/^[0-9]*$/", $Hprice)) {
        header("Location: ../Uploadhostel.php?error=invalidvalue");
        exit();
}
    elseif (!preg_match("/^[+0-9]*$/", $Hcontact)) {
        header("Location: ../Uploadhostel.php?error=invalidcontact");
        exit();
}
	...
	if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("Location: ../Uploadhostel.php?error=sqlerror");
		exit();
	}
	else {
		session_start();
		...
		header("Location: ../Uploadhostel.php?upload=success");
		exit();
	}
