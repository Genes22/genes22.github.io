...
if (isset($_POST['Update_uContact'])) {
    $contact = clean($_POST['uContact']);
    $id = $_SESSION['id'];
    $update = $conn->prepare("UPDATE users SET uContact=? WHERE idUsers=?");
    if ($update->execute(array($contact, $id))) {
        $msg = 'Contact Changed';
    }else{
        $error = 'Failed to change the contact';
    }
}
?>
...
<div style="color:rgb(254,254,254);">
    ...
    <div class="login-dark" style="background-image:url(&quot;assets/img/home.jpg&quot;);height:511px;">
        <form action="UpdateuContact.php" method="post" style="margin-top:-39px;background-color:rgba(30,40,51,0.67);">
            <h2 style="font-size:19px;margin-top:-22px;margin-bottom:45px;height:22px;"><strong>Update your infomation</strong></h2>
            <div class="form-group"><input class="form-control" type="number" name="uContact" minlength="10" maxlength=13 placeholder="Enter new contact" autofocus="" style="background-color:#ffffff;color:rgb(0,0,0);" required></div>
            <?php if (isset($msg)): ?>
            ...
            <?php endif ?>
            <div class="form-group"><button class="btn btn-primary active btn-block btn-sm" name="Update_uContact" type="submit"><strong>Update</strong></button></div>
        </form>
    </div>
