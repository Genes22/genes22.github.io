...
if (isset($_POST['Update_lname'])) {
    $last = clean($_POST['lName']);
    $id = $_SESSION['id'];
    $update = $conn->prepare("UPDATE users SET lName=? WHERE idUsers=?");
    if ($update->execute(array($last, $id))) {
        $msg = 'Lastname Changed';
    }else{
        $error = 'Failed to change the Lastname';
    }
}
?>
...
<div style="color:rgb(254,254,254);">
    ...
    <div class="login-dark" style="background-image:url(&quot;assets/img/home.jpg&quot;);height:511px;">
        <form action="UpdatelName.php" method="post" style="margin-top:-39px;background-color:rgba(30,40,51,0.67);">
            <h2 style="font-size:19px;margin-top:-22px;margin-bottom:45px;height:22px;"><strong>Update your infomation</strong></h2>
            <div class="form-group"><input class="form-control" type="text" name="lName" placeholder="Enter new Last Name" autofocus="" style="background-color:#ffffff;color:rgb(0,0,0);" required></div>
            <?php if (isset($msg)): ?>
            ...
            <?php endif ?>
            <div class="form-group"><button class="btn btn-primary active btn-block btn-sm" name="Update_lname" type="submit"><strong>Update</strong></button></div>
        </form>
    </div>
